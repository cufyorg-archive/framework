...
/**
 * A list specialized for {@code float} values.
 *
 * @author LSafer
 * @version 0.1.5
 * @since 0.1.5 ~2020.09.01
 */
public interface FloatList extends FloatCollection, PrimitiveList<
		Float,
		FloatConsumer,
		FloatPredicate,
		FloatUnaryOperator,
	...
	default int indexOf(Object object) {
		return object instanceof Float ? this.indexOf((float) object) : -1;
	}

	@Override
	default int lastIndexOf(Object object) {
		return object instanceof Float ? this.lastIndexOf((float) object) : -1;
	}

	@Override
	default Float remove(int index) {
		return this.removeFloatAt(index);
	}
	...
	default void replaceAll(UnaryOperator<Float> operator) {
		...
		this.replaceAll(
				operator instanceof FloatUnaryOperator ?
	...
	default void replaceAll(FloatUnaryOperator operator) {
		Objects.requireNonNull(operator, "operator");
		FloatListIterator iterator = this.listIterator();
		while (iterator.hasNext())
			iterator.set(operator.applyAsFloat(iterator.next()));
	}
	...
	default void sort(Comparator<? super Float> comparator) {
		this.sort(
				comparator == null ||
				comparator instanceof FloatComparator ?
