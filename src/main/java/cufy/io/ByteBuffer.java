...
public class ByteBuffer {
	 ...
	 */
	public ByteBuffer(int initialSize, int chunkSize) {
		if (initialSize < 1)
	 ...
	 */
	public ByteBuffer duplicate(int off, int chunkSize) {
		if (off > this.size)
		...
		ByteBuffer buffer = new ByteBuffer(length, chunkSize);
	...
	protected void expand(int length) {
		...
		if (length > this.data.length) {
			byte[] nio = new byte[length + this.chunkSize];
