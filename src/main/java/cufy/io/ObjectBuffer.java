 ...
 *
 * @param <T> the type of data this buffer is storing
 * @author lsafer
 * @version 0.1.3
 * @since 9-Jan-2020
 */
public class ObjectBuffer<T> {
	 ...
	 */
	public ObjectBuffer(int initialSize, int chunkSize) {
		if (initialSize < 1)
	 ...
	 */
	public ObjectBuffer duplicate(int off, int chunkSize) {
		if (off > this.size)
		...
		ObjectBuffer buffer = new ObjectBuffer(length, chunkSize);
	 ...
	 */
	public T read() {
		return this.cursor < this.size ? (T) this.data[this.cursor++] : null;
	}
	 ...
	 */
	public int read(T[] buffer, int off, int len) {
	 ...
	 * @throws IndexOutOfBoundsException if (off &gt;= cbuf.length). Or (off + len &gt;= cbuf.length). Or (off &lt; 0). Or (len &lt; 0)
	 */
	public void write(T[] buffer, int off, int len) {
		Objects.requireNonNull(buffer, "array");
	...
	protected void expand(int length) {
		...
		if (length > this.data.length) {
			Object[] nio = new Object[length + this.chunkSize];
