...
public class CharBuffer {
	 ...
	 */
	public CharBuffer(int initialSize, int chunkSize) {
		if (initialSize < 1)
	 ...
	 */
	public CharBuffer duplicate(int off, int chunkSize) {
		if (off > this.size)
		...
		CharBuffer buffer = new CharBuffer(length, chunkSize);
	...
	protected void expand(int length) {
		...
		if (length > this.data.length) {
			char[] nio = new char[length + this.chunkSize];
